...
void LCD_String_xy (char row, char pos, char *str)	/* Send string to LCD function */
{
	if (row == 1)
		...
		LCD_Command(pos + 0xC0);				/* Command of Second row and required position<16 */
	else if (row == 3)
		LCD_Command(pos + 0x94);				/* Command of Third row and required position<16 */
	else if (row == 4)
		LCD_Command(pos + 0xD4);				/* Command of Fourth row and required position<16 */
