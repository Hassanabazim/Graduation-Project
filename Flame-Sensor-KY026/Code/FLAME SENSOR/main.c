...
int main()
{
	...
	while(1)
	{
		flame = ADC_readChannel(0);
		double percentage = (flame / 1023.00 ) * 100 ;
		...
		_delay_ms(50);
		 ...
		 if ((flame < safe) && ( flame > danger))
		 {
				PORTC = 0b000001000;
				LCD_String_xy(2,0,"FIRE ZONE");
				_delay_ms(100);
		 }
		else if (flame <= danger)
		 {
				PORTC = 0b000001000;
